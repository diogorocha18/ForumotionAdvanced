(function() {
  ...
  FAE.cp_rev = '1.4.1';
  FAE.raw = 'https://raw.githubusercontent.com/diogorocha18/ForumotionAdvanced/master/';
  ...
  FAE.getUpdates = function() {
    if (++FAE.update_index >= FAE.update_quota) {
      ...
      $.get(FAE.raw + 'updates/' + FAE.update_queue[FAE.update_index] + '.js', function(d) {
        ...
        }) : 'Update instructions for version ' + FAE.update_queue[FAE.update_index] + ' could not be found. Please <a href="https://github.com/diogorocha18/ForumotionAdvanced/wiki/Support-and-Discussion" target="_blank">open a new issue</a> and provide this information for further assistance.', 'color:#E53;font-weight:bold;');
  ...
  FAE.ready(function() {
    var admin = $('a[href^="/admin/"]').not('.mainmenu')[0], // get the AP link so we can fetch the TID
        ...
        installed = document.getElementById('fa_advanced');
    ...
    if (admin && !/page_html\?mode=preview/.test(window.location.href)) {
      ...
      if (founder) {
        ...
        document.getElementById('fae_install').onclick = function() {
          ...
          }) : 'Are you sure you want to ' + ( installed ? 're' : '' ) + 'install Forumotion Advanced? This will overwrite your current theme and delete your current JavaScripts. \\\n\\\nPlease make sure to backup all your personal content files such as CSS, Templates, and JavaScripts before proceeding. Click "Cancel" if you\'re not ready to install Forumotion Advanced.').replace(/\\/g, '') )) {
      ...
      $.get('/admin/', function(d) {
        if (!$('#page-body > #tabs', d)[0]) {
          FAE.log('Error : You have not logged into your <a href="/admin/">administration panel</a>. Please log in so that you can use the FAE Control Panel. <a href="https://github.com/diogorocha18/ForumotionAdvanced/wiki/Frequently-Asked-Questions#wiki-wrapper" target="_blank" style="font-weight:normal;"><em>(What is this?)</em></a>', 'color:#E53;font-weight:bold;');
          document.getElementById('fae_options').style.display = 'none';
        }
      });
      ...
      if (installed) {
        ...
        uninstall.onclick = function() {
          if (confirm( (FAE.cp_lang.fae_uninstall_warning || 'Are you sure you want to uninstall Forumotion Advanced? All CSS, JavaScript, and Template changes will be deleted ; The forum will be reverted to the default phpbb3 theme. \\\n\\\nPlease make sure to backup all your personal content files such as CSS, Templates, and JavaScripts before proceeding. Click "Cancel" if you don\'t want to uninstall Forumotion Advanced yet.').replace(/\\/g, '') )) {
        ...
        update.onclick = function() {
          ...
          $.get(FAE.raw + 'javascripts/version-data.js', function(d) {
            ...
            if (forumactif_edge_version_data.length < fae_github_version_data.length) {
              ...
              FAE.log(FAE.cp_lang.fae_update_good || 'Forumotion Advanced is up to date!', 'color:#8B5;font-weight:bold;');
        ...
        document.getElementById('fae_translate').onclick = function() {
          ...
          if (select.value == 'ADD') {
            if (confirm(FAE.cp_lang.fae_translate_submit_confirm || 'You have chosen to submit a new translation. If this is correct, please click "OK" and proceed to the translation page, otherwise click "cancel" and choose another language.')) {
              window.location.href = 'https://github.com/diogorocha18/ForumotionAdvanced/wiki/Translating#why-isnt-x-language-available-';
            }
          ...
          if (FAE.board_lang == selected.innerHTML) {
            ...
            }) : 'Forumotion Advanced is already in ' + selected.innerHTML + '. Please choose another language.');
          }

          if (confirm(FAE.cp_lang.fae_translate_warning ? FAE.parse_vars(FAE.cp_lang.fae_translate_warning, {
            '{LANG}' : selected.innerHTML

          }) : 'Are you sure you want to change the language of Forumotion Advanced to ' + selected.innerHTML + ' ?')) {

            FAE.log(FAE.cp_lang.fae_translate_start || 'Translation of Forumotion Advanced will commence shortly. Please wait..');
        ...
        $(opts).append('<div id="category-general" style="display:block;">'+
          ...
          '<p id="fae_gen_desc">This section allows you to manage the general settings of Forumotion Advanced.</p>'+
        ...
        $(opts).append('<div id="category-theme" style="display:none;">'+
          ...
          '<p id="fae_theme_desc">This section allows you to manage the default theme and colors for Forumotion Advanced by importing a new theme from the Github repository.</p>'+
          ...
          '<div class="fae_cp_row">'+
            ...
            '<select id="fae_selected_theme">'+
              '<option value="fa_advanced" selected>Edge Default</option>'+
              '<option value="fa_advanced-dark">Edge Dark</option>'+
            '</select>'+
          '</div>'+
        ...
        $(opts).append('<div id="category-color" style="display:none;">'+
          ...
          '<div class="fae_options_column">'+
            ...
            '<p id="fae_colors_desc-primary">The options below allow you to adjust the 5 primary color shades for Forumotion Advanced.</p>'+
        ...
        $(opts).append('<div id="category-plugin" style="display:none;">'+
          ...
          '<p id="fae_plugin_desc">This section allows you to manage the settings of core-plugins for Forumotion Advanced.</p>'+
        ...
        $.get(FAE.raw + 'lang/' + document.getElementById('fae_selected_language').value + '.js', function(d) {
          ...
          if (!founder && !installed) {
            ...
            FAE.log(FAE.cp_lang.co_admin || 'Only <a href="/u1">the founder</a> can access the installation, update, and translation actions. Since Forumotion Advanced is installed, you can still access some core features for personalizing your forum via the "Configuration" section below.', 'color:#C93;font-weight:bold;');
          }
          ...
          $.get(FAE.raw + 'javascripts/version-data.js', function (d) {
            try {
              ...
              }) : 'Your installed version of Forumotion Advanced is <a href="https://github.com/diogorocha18/ForumotionAdvanced/releases/tag/' + local + '" target="_blank">' + local + '</a>.');
              ...
              }) : 'The current release of Forumotion Advanced on Github is <a href="https://github.com/diogorocha18/ForumotionAdvanced/releases/tag/' + github + '" target="_blank">' + github + '</a>.');

              // log version status
              FAE.log(
                equal ? (FAE.cp_lang.fae_update_good || 'Forumotion Advanced is up to date!') :
                        (FAE.cp_lang.update_waiting || 'There are updates available for Foruactif Edge. Check out <a href="https://github.com/diogorocha18/ForumotionAdvanced/wiki/Checking-for-updates" target="_blank">the wiki</a> for more information about updating.'),
                'color:' + ( equal ? '#8B5' : '#C93' ) + ';font-weight:bold;'
              );

              // log release list
              FAE.log(FAE.cp_lang.release_list || '<a href="https://github.com/diogorocha18/ForumotionAdvanced/releases" target="_blank">Click here</a> to see the full list of releases for Forumotion Advanced.');
              ...
              FAE.log('An error has occurred while trying to retrieve Forumotion Advanced\'s version data. Please make sure JS Codes Management is enabled. If it is please <a href="https://github.com/diogorocha18/ForumotionAdvanced/wiki/Support-and-Discussion" target="_blank">open a new issue</a> and provide this information for further assistance.', 'color:#E53;font-weight:bold;');
      ...
      // add installation options if not installed
        ...
        $('#fae_options').append(
          ...
          '<p id="fae_install_options_desc">Use the options below to configure the installation of Forumotion Advanced.</p>'+

          '<div class="fae_cp_row">'+
            '<span class="fae_help_me">?'+
              '<span class="fae_help_tip">The install lanugage of Forumotion Advanced. This can be changed at anytime after installation.</span>'+
            '</span>'+
  ...
  $('#fae_cp').append(
    '<div style="margin-top:12px">'+
      ...
      '<a href="https://github.com/diogorocha18/ForumotionAdvanced/wiki/FAE-Control-Panel-Guide" target="_blank" style="float:right;"><strong id="fae_cp_help">Help!</strong></a>'+
      '<div class="clear"></div>'+
    '</div>'
  );
  ...
  // maintenance message
  if (FAE.maintenance) {
    $('#fae_cp').prepend('<div id="fae_maintenance"><i class="fa fa-warning"></i>Maintenance is currently being performed on the Control Panel. Don\'t worry if you notice anything strange, we\'ll get things back to normal shortly. For more information, please read about "<a href="https://github.com/diogorocha18/ForumotionAdvanced/wiki/Maintenance" target="_blank">Maintenance</a>" on our wiki.</div>');
  }
